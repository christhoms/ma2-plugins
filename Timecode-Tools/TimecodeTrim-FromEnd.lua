function trimTimecodeEnd(tcNum, timeFromStart, timeFromEnd, destTC)
    ...
    for i = 1, #t do
        if t[i]:find('lenght="%d+"') then
            _, _, length = t[i]:find('lenght="(%d+)"')
            length = tonumber(length)
            gma.echo("Timecode Length: " .. length)
        endTrim = length - tonumber(timeFromEnd)
        end
...
return function()
    ...
    trimTimecodeEnd(inputTC, 0, timeFromEnd, destTC)
end